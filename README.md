...
This repo is built in the PRD's phases (Section 7). **Current status: Phase 3 — Logic (complete).**
Account/deploy setup is in [`docs/MANUAL_SETUP.md`](docs/MANUAL_SETUP.md).
...
- **Two Lambdas** (Node 20, arm64): the router (all app routes) and the RevenueCat webhook.

## Phase 3 — what's in the logic

The router + webhook handlers ([`src/handlers`](src/handlers), with helpers in [`src/lib`](src/lib)):

- **AI proxy** (`/identify`, `/diagnose`) via `@google/genai` structured output, with the
  accuracy/safety rules enforced server-side (low confidence → "Unknown Plant" + null cadence;
  unknown toxicity → "High"). Images are forwarded inline and never stored.
